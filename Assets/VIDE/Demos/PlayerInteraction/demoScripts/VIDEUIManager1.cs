...
public class VIDEUIManager1 : MonoBehaviour
{
    ...
    public AudioClip apolloMellody;
    ...
    public void SetOptions(string[] choices)
    {
        ...
        for (int i = 0; i < choices.Length; i++)
        {
            ...
            newOp.GetComponent<RectTransform>().anchoredPosition = new Vector2(50, 10 - (20 * i));
    ...
    bool PreConditions(VIDE_Assign dialogue)
    {
        ...
        } else //Stuff we do right before the dialogue begins
        {
            switch (dialogue.alias)
            {
                ...
                case "Zeus":
                    var.metZeus = true;
                    if (var.metEveryone && !var.returnedToZeus)
                    {
                        dialogue.overrideStartNode = 5;
                        var.returnedToZeus = true;
                    }
                    else if(var.returnedToZeus && !var.returnedToZeusOnce)
                    {
                        dialogue.overrideStartNode = 7;
                        var.returnedToZeusOnce = true;
                    }
                    break;
                ...
                case "Artemis":
                    var.metArtemis= true;
                    if (var.numArrows == 3 && !var.returnedToArtemis && var.metArtemisOnce)
                    {
                        dialogue.overrideStartNode = 6;
                        var.returnedToArtemis = true;
                    }
                    break;
    ...
    void GiveItem(int itemIndex)
    {
        ...
        string text = "You have received <color=yellow>" + player.demo_Items[itemIndex] + "</color>!";
